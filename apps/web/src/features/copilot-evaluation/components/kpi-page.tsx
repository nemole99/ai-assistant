...
export function CopilotEvaluationKpi() {
  ...
  const { data: ticketResult } = useTickets({ limit: 100, month });
  ...
  const stats = useMemo(() => {
    ...
    const devTicketCounts = tickets.reduce(
      (acc, t) => {
    ...
    const maxDev = Object.entries(devTicketCounts).toSorted(
      ([, a], [, b]) => b - a
    )[0];
            ...
            <CardContent className="overflow-x-auto">
              <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                  {(summary ?? []).map((row, idx) => {
                    const prodBelow =
                      row.resultProductivity !== undefined &&
                      row.resultProductivity !== null &&
                      row.targetProductivity !== undefined &&
                    ...
                    {MONTHS.map((m) => (
                      <TableHead
                        ...
                        className={cn(
                          ...
                          m === selectedMonthKey && "bg-primary/10 font-bold"
                        )}
                      >
                        {m}
                      </TableHead>
